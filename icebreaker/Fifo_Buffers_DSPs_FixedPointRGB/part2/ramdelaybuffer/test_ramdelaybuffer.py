...
@pytest.mark.parametrize("CycleDelay", [4, 2])
@pytest.mark.parametrize("test_name", tests)
def test_each_runner(test_name, DataWidth, CycleDelay):
...
@pytest.mark.parametrize("DataWidth", [7, 32])
@pytest.mark.parametrize("CycleDelay", [4, 1])
@max_score(1)
def test_all_runner(DataWidth, CycleDelay):
    # This line must be first
    parameters = dict(locals())
    runner(timescale, DIR_PATH, None, parameters)

@pytest.mark.parametrize("DataWidth", [32])
@pytest.mark.parametrize("CycleDelay", [4])
@pytest.mark.skipif(not os.getenv('SIM').lower().startswith("verilator"), reason="Requires Verilator")
@max_score(.5)
def test_lint(DataWidth, CycleDelay):
    # This line must be first
    parameters = dict(locals())
...
@pytest.mark.skipif(not os.getenv('SIM').lower().startswith("verilator"), reason="Requires Verilator")
@max_score(.5)
def test_style(DataWidth, CycleDelay):
    # This line must be first
    parameters = dict(locals())
...
class BufferModel():
    def __init__(self, dut):
        ...
        self._CycleDelay = dut.CycleDelay.value
        self._buf = [0] * self._CycleDelay
        self._deqs = 0
        self._enqs = 0
    ...
    def produce(self):
        ...
        if(self._deqs >= self._dut.CycleDelay.value):
...
async def out_fuzz_test(dut):
    """Transmit 4 * CycleDelay random data elements at 50% line rate (Output/Consumer is fuzzed)"""

    l = (1 << dut.CycleDelay.value) * 4
    rate = .5
...
async def in_fuzz_test(dut):
    """Transmit 4 * CycleDelay random data elements at 50% line rate (Input/Producer is fuzzed)"""
...
async def inout_fuzz_test(dut):
    """Transmit 4 * CycleDelay random data elements at ~25% line rate (Both are fuzzed)"""
...
async def full_bw_test(dut):
    """Transmit 8 * CycleDelay random data elements at 100% line rate"""

    # This is the InputModel
    l = (1 << dut.CycleDelay.value) * 8
    rate = 1
