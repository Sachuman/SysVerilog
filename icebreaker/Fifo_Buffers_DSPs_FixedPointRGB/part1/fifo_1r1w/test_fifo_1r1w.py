...
@pytest.mark.parametrize("DataWidth", [7, 32])
@pytest.mark.parametrize("NumEntriesLog2", [4, 2])
@pytest.mark.parametrize("test_name", tests)
def test_each_runner(test_name, DataWidth, NumEntriesLog2):
...
@pytest.mark.parametrize("NumEntriesLog2", [4, 2])
@max_score(1)
def test_all_runner(DataWidth, NumEntriesLog2):
    # This line must be first
    parameters = dict(locals())
...
@pytest.mark.parametrize("NumEntriesLog2", [4, 2])
@max_score(1)
def test_bypass_runner(DataWidth, NumEntriesLog2):
    # This line must be first
    parameters = dict(locals())
...
@max_score(.125)
def test_lint(DataWidth, NumEntriesLog2):
    # This line must be first
    parameters = dict(locals())
    lint(timescale, DIR_PATH, ["--lint-only"], parameters)

@pytest.mark.parametrize("DataWidth", [7])
@pytest.mark.parametrize("NumEntriesLog2", [4])
@pytest.mark.skipif(not os.getenv('SIM').lower().startswith("verilator"), reason="Requires Verilator")
@max_score(.5)
def test_style(DataWidth, NumEntriesLog2):
    # This line must be first
    parameters = dict(locals())
...
class FifoModel():
    def __init__(self, dut):
        ...
        self._DataWidth = dut.DataWidth.value
        self._NumEntriesLog2 = dut.NumEntriesLog2.value
        self._deqs = 0
        self._enqs = 0
...
class RandomDataGenerator():
    ...
    def generate(self):
        value = random.randint(0, (1 << self._dut.DataWidth.value) - 1)
        return value
...
async def fill_test(dut):
    """Test if fifo_1r1w fills to NumEntries elements"""

    NumEntries = (1 << dut.NumEntriesLog2.value)
    l = NumEntries
    rate = 1
    ...
    success = False
    try:
        await im.wait(NumEntries)
        success = True
    except:
        nconsumed = im.nconsumed()

    if(not success):
        assert nconsumed != NumEntries, f"Error! Could not fill fifo with {NumEntries} elements in {NumEntries} cycles. Fifo consumed {nconsumed} elements."
...
async def fill_empty_test(dut):
    ...
    success = False
    try:
        await om.wait(NumEntries)
        success = True
    except:
        nproduced = om.nproduced()

    if(not success):
        assert nproduced != NumEntries, f"Error! Could not empty fifo with {NumEntries} elements in {NumEntries} cycles. Fifo produced {nproduced} elements."

@cocotb.test()
async def out_fuzz_test(dut):
    """Transmit 4 * NumEntries random data elements at 50% line rate (Output/Consumer is fuzzed)"""

    l = (1 << dut.NumEntriesLog2.value) * 4
    rate = .5
...
async def in_fuzz_test(dut):
    """Transmit 4 * NumEntries random data elements at 50% line rate (Input/Producer is fuzzed)"""
...
async def inout_fuzz_test(dut):
    """Transmit 4 * NumEntries random data elements at ~25% line rate (Both are fuzzed)"""
...
async def full_bw_test(dut):
    """Transmit 8 * NumEntries random data elements at 100% line rate"""

    # This is the InputModel
    l = (1 << dut.NumEntriesLog2.value) * 8
    rate = 1
